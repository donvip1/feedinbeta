-- Fix get_feed_with_rotation to use correct schema (boost_level instead of is_promoted)
...
BEGIN
  ...
  promoted_posts AS (
    ...
    WHERE pp.status = 'active' AND pp.end_date > NOW()
  ),
    ...
    SELECT 
      ...
      (
        ...
        CASE 
          ...
          ELSE 0
        END +
        -- Promoted content boost (using boost_level)
        CASE 
          WHEN p.boost_level = 'premium' THEN 50
          WHEN p.boost_level = 'standard' THEN 30
          WHEN p.id IN (SELECT promoted_posts.post_id FROM promoted_posts) THEN 20
          ELSE 0
        END
      ...
      -- Check if promoted
      (p.boost_level IS NOT NULL OR p.id IN (SELECT promoted_posts.post_id FROM promoted_posts)) AS is_promoted
    FROM posts p
...
-- Also fix get_randomized_feed_cycle
...
BEGIN
  ...
  SELECT 
    ...
    p.post_type,
    p.original_post_id,
    (p.boost_level IS NOT NULL) AS is_promoted
  FROM posts p
  WHERE p.status = 'active'
    AND p.user_id NOT IN (SELECT blocked.blocked_id FROM blocked)
    AND (
