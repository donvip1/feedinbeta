-- Update the rotation function to increment session position after each fetch
-- This ensures users get different starting positions when they return
...
BEGIN
  ...
  DO UPDATE SET 
    last_position = user_feed_sessions.last_position + p_limit,
    session_start = NOW();
...
-- Create unique constraint for session tracking if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_feed_sessions_user_feed_type_key'
  ) THEN
    ALTER TABLE user_feed_sessions ADD CONSTRAINT user_feed_sessions_user_feed_type_key 
