-- Fix send_gift function to increment gifts_count by 1 instead of credit_value
-- This fixes the bug where a 10-heart gift counts as 10 gifts instead of 1
...
BEGIN
  ...
  -- FIX: Update post gifts count by 1 (not credit value!)
...
-- Also recalculate any existing incorrect gift counts from gift_analytics
UPDATE posts p
SET gifts_count = COALESCE(
  (SELECT COUNT(*) FROM gift_analytics ga WHERE ga.source_id = p.id AND ga.source_type = 'post'),
  0
)
WHERE p.id IN (SELECT DISTINCT source_id FROM gift_analytics WHERE source_type = 'post');