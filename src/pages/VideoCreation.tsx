  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select("daily_ai_video_count, last_ai_reset")
        .eq("id", user.id)
        .single();
      ...
      if (!isAdmin) {
        ...
        const currentCount = lastReset === today ? profile?.daily_ai_video_count || 0 : 0;

        if (currentCount >= 1) {
          if (!credits || credits.balance < 50) {
            toast({
              title: "Daily limit reached",
              description: "You need 50 credits to continue or wait until tomorrow",
              variant: "destructive"
            });
            setLoading(false);
            return;
...
- Video Title
- Target Duration
- Shot List (with scene descriptions)
- Script/Narration
- Visual Effects/Transitions
- Music/Sound Recommendations
- Editing Notes

Provide practical, actionable guidance for video creation.`;
      ...
      if (!isAdmin) {
        ...
        if (currentCount >= 1 && credits && credits.balance >= 50) {
        ...
        await supabase
          .from("profiles")
          .update({
            daily_ai_video_count: currentCount + 1,
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
        <div className="container max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold">Video Creation</h1>
          </div>
        </div>
      </div>
      ...
      <div className="container max-w-2xl mx-auto px-4 py-6 space-y-6">
        <div className="bg-card rounded-lg p-4 border">
          <p className="text-sm text-muted-foreground mb-4">
            Describe your video idea and get a complete production plan with script, shot list, and editing notes.
          </p>
          <Textarea
            placeholder="E.g., '5-minute tutorial video on how to make homemade pasta'"
            value={prompt}
        ...
        {result && (
          <div className="bg-card rounded-lg p-4 border">
            <h3 className="font-semibold mb-3">Video Production Plan:</h3>
