  ...
  const acceptFriendRequest = async (senderId: string, senderName: string) => {
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        message: `${myProfile?.display_name || myProfile?.username || 'Someone'} accepted your friend request`,
        related_id: user.id,
        related_type: 'profile'
      });
  ...
  const declineFriendRequest = async (senderId: string, senderName: string) => {
    ...
    try {
      ...
      await supabase.from('notifications').insert({
        ...
        message: `${myProfile?.display_name || myProfile?.username || 'Someone'} declined your friend request`,
        related_id: user.id,
        related_type: 'profile'
      });
