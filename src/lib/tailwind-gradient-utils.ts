/**
 * Utility to convert Tailwind gradient classes to CSS gradient values
 * Used for text_styled posts where the media_url stores Tailwind classes
 */
...
const tailwindColors: Record<string, string> = {
  ...
  'fuchsia-600': '#c026d3', 'fuchsia-700': '#a21caf', 'fuchsia-800': '#86198f', 'fuchsia-900': '#701a75',
 ...
 */
function getColorFromClass(colorClass: string): string | null {
  // Extract color from "from-purple-600" or "to-pink-600" format
  const match = colorClass.match(/(?:from|to|via)-(\w+-\d+)/);
  if (match) {
    return tailwindColors[match[1]] || null;
  }
  return null;
}
 ...
 * @param tailwindClass - The Tailwind class string (e.g., "bg-gradient-to-br from-purple-600 to-pink-600")
 * @returns CSS gradient value for use in style.background
 */
export function tailwindGradientToCSS(tailwindClass: string): string {
  if (!tailwindClass) {
  ...
  // Check for solid color backgrounds first
  for (const cls of classes) {
    if (cls.startsWith('bg-') && !cls.includes('gradient')) {
      // Extract color from "bg-purple-600" format
      const colorMatch = cls.match(/bg-(\w+-\d+)/);
  ...
  for (const cls of classes) {
    if (cls.startsWith('from-')) {
      fromColor = getColorFromClass(cls);
    } else if (cls.startsWith('via-')) {
      viaColor = getColorFromClass(cls);
    } else if (cls.startsWith('to-')) {
      toColor = getColorFromClass(cls);
    }
  }
  ...
  if (fromColor && toColor) {
    if (viaColor) {
      return `linear-gradient(${direction}, ${fromColor} 0%, ${viaColor} 50%, ${toColor} 100%)`;
    }
    return `linear-gradient(${direction}, ${fromColor} 0%, ${toColor} 100%)`;
  }

  if (fromColor) {
    return `linear-gradient(${direction}, ${fromColor} 0%, ${fromColor}88 100%)`;
  }
...
export default tailwindGradientToCSS;
