...
void main() {
  ...
  testWidgets('immersive post renders one author header in the caption overlay', (
    tester,
  ) async {
    ...
    // Immersive composition surfaces exactly one creator header carrying the
    // author identity, placed in the bottom caption overlay (not a top bar).
    final header = find.byType(CreatorHeader);
    expect(header, findsOneWidget);
    expect(find.text('Top Creator'), findsOneWidget);
    expect(tester.getTopLeft(header).dy, greaterThan(400));
  });
}
